...
export const FutureComparison = memo(({ data, onReset }: Props) => {
  ...
  const activeData = data.scenarios.find((s) => s.scenario_id === activeScenario)
  ...
  return (
    <div className="animate-fade-in space-y-8">
      ...
      {/* Footprint breakdown */}
      <section aria-label="Current footprint breakdown" className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        <h3 className="text-base font-semibold text-gray-800 mb-4">Current Footprint Breakdown</h3>
            ...
            return (
              <div key={key}>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <dd className="text-gray-500">{formatTons(val)} ({pct}%)</dd>
                </div>
                <div className="h-3 bg-gray-100 rounded-full overflow-hidden" role="presentation">
                  <div
                    className={`h-full rounded-full transition-all duration-700 ${BAR_COLORS[key] ?? 'bg-gray-400'}`}
                    style={{ width: `${pct}%` }}
                    aria-valuenow={pct}
                    aria-valuemin={0}
                    aria-valuemax={100}
        ...
        <div role="tablist" aria-label="Select future scenario" className="grid grid-cols-3 gap-3 mb-6">
          {data.scenarios.map((s) => (
            <button
              ...
              className={`rounded-xl border-2 p-4 text-left transition-all focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-1 ${
                activeScenario === s.scenario_id
                  ? 'border-green-500 shadow-md scale-105'
                  : 'border-gray-200 hover:border-gray-300'
              } bg-gradient-to-br ${scenarioGradient(s.scenario_id)}`}
            >
              <div className="text-2xl mb-2" aria-hidden="true">{SCENARIO_ICONS[s.scenario_id]}</div>
      ...
      <div className="text-center pt-4">
        <button
          onClick={onReset}
          className="px-6 py-3 border-2 border-gray-300 text-gray-700 rounded-xl hover:border-gray-400 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 transition-colors font-medium"
